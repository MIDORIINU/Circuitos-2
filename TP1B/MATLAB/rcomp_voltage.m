function rcomp_voltage(spice_directory, loop_directory, rf_directory, ...
...
component_values = {'0', '10', '100'};
component_labels = {'0', '10\Omega', '100\Omega'};
...
simulation_name = 'power_supply_RCOMP_LOOP_Modo1';

simulation_directory = 'Rcomp_tensión';

parametric_labels = {'0', '10\Omega', '100\Omega'};

simulation_title = strjoin({'Ganancia de lazo ', ...
    '$ \left( a \cdot f \right)', ...
    '_{ \left( j \cdot \omega \right)} $', ...
    ' parametrizada por Rcomp en modo tensi\''{o}n, 10V, carga de 1A'}, '');
...
simulation_name = 'power_supply_RCOMP_LOOP_Modo2';

simulation_title = ...
    strjoin({'Ganancia de lazo $ \left( a \cdot f \right)', ...
    '_{ \left( j \cdot \omega \right)} $', ...
    ' parametrizada por Rcomp en modo tensi\''{o}n, 1V, carga de 1A'}, '');
...
simulation_name = 'power_supply_RCOMP_RF_Modo1';
...
simulation_ang_limits = [-300 60];

simulation_ang_ticks = unique(sort([(-300 :30: 60) 0]));
...
simulation_name = 'power_supply_RCOMP_RF_Modo2';
...
simulation_name = sprintf('power_supply_RCOMP_%s_STEP_Modo1', ...
    component_value);
...
simulation_voltage_limits = [0 14];

simulation_voltage_ticks = (0 :1:14);
...
simulation_name = sprintf('power_supply_RCOMP_%s_STEP_Modo2', ...
    component_value);
