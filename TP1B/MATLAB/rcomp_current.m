function rcomp_current(spice_directory, loop_directory, rf_directory, ...
...
simulation_name = 'power_supply_RCOMP_LOOP_Modo3';

simulation_directory = 'Rcomp_corriente';
...
simulation_title = strjoin({'Ganancia de lazo ', ...
    ...
    ' parametrizada por Rcomp en modo corriente, 2A'}, '');
...
simulation_name = 'power_supply_RCOMP_LOOP_Modo4';

simulation_title = ...
    strjoin({'Ganancia de lazo $ \left( a \cdot f \right)', ...
    '_{ \left( j \cdot \omega \right)} $', ...
    ' parametrizada por Rcomp en modo corriente, 200mA'}, '');
...
simulation_name = 'power_supply_RCOMP_RF_Modo3';
...
simulation_mod_limits = [-65 55];

simulation_mod_ticks = (-65:10:55);

simulation_ang_limits = [-300 90];

simulation_ang_ticks = unique(sort([(-300 :30: 90) 0]));
...
simulation_name = 'power_supply_RCOMP_RF_Modo4';
...
simulation_name = sprintf('power_supply_RCOMP_%s_STEP_Modo3', ...
    component_value);
...
simulation_name = sprintf('power_supply_RCOMP_%s_STEP_Modo4', ...
    component_value);
