...
\begin{itemize}
	\item Máxima Potencia de Salida:  $100\si[per-mode=symbol]{\watt}$ RMS @ $8\si[per-mode=symbol]{\ohm}$
	\item Salida clase G
	\item THD: $< 0.01 \% @ 1\si[per-mode=symbol]{\kilo\hertz}$, $<0.02 \% @ 10\si[per-mode=symbol]{\kilo\hertz},$ a $100W RMS@8\si[per-mode=symbol]{\ohm}$ y $1\si[per-mode=symbol]{\watt} RMS @8 \si[per-mode=symbol]{\ohm}$
	\item Slew Rate: $>15\frac{V}{\mu S}$
	\item Impedancia de entrada: $10\si[per-mode=symbol]{\kilo\ohm}$
	\item Sensibilidad: $1\si[per-mode=symbol]{\volt}$ RMS @ $8\Omega$
	...
	\item Alimentación: 
	\begin{itemize}
		\item Baja tensión: $ +/-15\si[per-mode=symbol]{\volt}$, riple máximo $5\%$
		\item Alta tensión: $ +/-50\si[per-mode=symbol]{\volt}$, riple máximo $5\%$
	\end{itemize}
\end{itemize}

\paragraph{Máxima excursión: $40\si[per-mode=symbol]{\volt}$ - } Para una salida senoidal de $100\si[per-mode=symbol]{\watt} RMS$, su potencia pico es $200\si[per-mode=symbol]{\watt} = \frac{V_{max}^2}{R_L}$ que, con carga $R_L=8\si[per-mode=symbol]{\ohm}$ da una tensión pico de $V_{max}=40\si[per-mode=symbol]{\volt}$. A esta tensión se llega cuando la entrada es la sensibilidad especificada, $1\si[per-mode=symbol]{\volt} RMS @8\si[per-mode=symbol]{\ohm}$, que se corresponde a $1\si[per-mode=symbol]{\volt}$ pico. Estos $40\si[per-mode=symbol]{\volt}$ serán la máxima excursión, la tensión máxima en la que el amplificador garantiza que no haya recortes considerables.

\paragraph{Nota sobre el slew-rate - } El slew rate especificado ($15\frac{V}{\mu s}$) duplica el valor mínimo para cumplir las otras especificaciones: el mayor ritmo de crecimiento para señales de ancho de banda $30\si[per-mode=symbol]{\kilo\hertz}$ y máxima excursión $40\si[per-mode=symbol]{\volt}$ se da cuando la senoide cruza por cero, y su pendiente es $2 \pi \times 30\si[per-mode=symbol]{\kilo\hertz}\times 40\si[per-mode=symbol]{\volt}\cong 7.5\frac{V}{\mu s}$. 
