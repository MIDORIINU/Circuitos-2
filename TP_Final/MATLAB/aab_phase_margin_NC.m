function aab_phase_margin_NC(spice_directory, points_directory, ...
...
simulation_name = 'amplifier_V2.1_LOOP_NC';

simulation_save_name = 'gain_loop_NC';

simulation_title = strjoin({'Ganancia de lazo ', ...
    '$ \left( a \cdot f \right)', ...
    '_{ \left( j \cdot \omega \right)} $'}, '');

simulation_mod_limits = [-180 90];

simulation_mod_ticks = (-180:10:90);

simulation_ang_limits = [-450 50];

simulation_ang_ticks = unique(sort([(-450 :30: 50) -180]), 'first');
